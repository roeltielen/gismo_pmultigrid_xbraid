...
gsPreconditionerOp<>::Ptr setupSubspaceCorrectedMassSmoother(const gsSparseMatrix<>&, const gsMultiBasis<>&, const gsBoundaryConditions<>&, const gsOptionList&, const int&);
...
public:
  ...
  virtual void solve(const gsMatrix<T> & rhs, vector<memory::shared_ptr<gsMultiBasis<T> > > m_basis , gsMatrix<T>& x, const int& numLevels, const int& numCoarsening, const int& numRefine, const int& numSmoothing, int& numCoarseCycles, const int& typeCycle_p, int& typeCycle_h, const int& typeSolver, const int& typeBCHandling, gsBoundaryConditions<T> bcInfo, gsMultiPatch<> mp, gsGeometry<>::Ptr geo, const int& typeLumping, const int& typeProjection, const int& typeSmoother, vector<gsSparseMatrix<T>>& m_prolongation_P, vector<gsSparseMatrix<T>>& m_restriction_P, vector<gsMatrix<T>>& m_prolongation_M,vector<gsMatrix<T>>& m_restriction_M,vector<gsSparseMatrix<T>>& m_prolongation_H, vector<gsSparseMatrix<T>>& m_restriction_H, const gsMatrix<>& hp)
  {
    if( numLevels == 1)
    {
    ...
    if(hp(max(numLevels-2,0),0) == 0 )
    {
      ...
      presmoothing(rhs, x, numLevels, numSmoothing, fineRes, numRefine, typeSmoother,hp); 
        restriction(fineRes, coarseRes, numLevels, numCoarsening, m_basis, typeLumping, typeBCHandling, bcInfo, mp, geo, typeProjection, m_prolongation_P, m_restriction_P,m_prolongation_M, m_restriction_M, m_prolongation_H, m_restriction_H,hp);
          coarseCorr.setZero(coarseRes.rows(),1);
            for( int j = 0 ; j < (typeCycle_p == 2 ? 2 : 1) ; j++)
            {   
              solve(coarseRes, m_basis, coarseCorr, numLevels-1, numCoarsening, numRefine, numSmoothing, numCoarseCycles, typeCycle_p, typeCycle_h, typeSolver, typeBCHandling, bcInfo, mp, geo, typeLumping, typeProjection, typeSmoother, m_prolongation_P, m_restriction_P,m_prolongation_M, m_restriction_M,m_prolongation_H, m_restriction_H,hp);  
            }
        prolongation(coarseCorr, fineCorr, numLevels, numCoarsening, m_basis, typeLumping, typeBCHandling, bcInfo, mp, geo, typeProjection, m_prolongation_P, m_restriction_P,m_prolongation_M, m_restriction_M,m_prolongation_H, m_restriction_H,hp);
  ...
  virtual void presmoothing(const gsMatrix<T>& rhs, gsMatrix<T>& x, const int& numLevels, const int& numSmoothing, gsMatrix<T> & fineRes , const int& numRefine, const int& typeSmoother, const gsMatrix<>& hp) = 0;

  /// @brief Apply fixed number of smoothing steps (pure virtual method)
  virtual void postsmoothing(const gsMatrix<T>& rhs, gsMatrix<T>& x, const int& numLevels, const int& numSmoothing, gsMatrix<T> & fineCorr, gsMatrix<T> & postRes, const int& typeSolver, const int& numRefine, const int& typeSmoother, const gsMatrix<>& hp) = 0;
  ...
  virtual gsSparseMatrix<T> prolongation_P(const int& numLevels, vector<memory::shared_ptr<gsMultiBasis<T> > > m_basis, const int& typeLumping, const int& typeBCHandling, gsGeometry<>::Ptr geo, const int& typeProjection) = 0;
   
  /// @brief Prolongate coarse space function to fine space
  virtual gsSparseMatrix<T> restriction_P(const int& numLevels, vector<memory::shared_ptr<gsMultiBasis<T> > > m_basis, const int& typeLumping, const int& typeBCHandling, gsGeometry<>::Ptr geo, const int& typeProjection) = 0;
  
  /// @brief Prolongate coarse space function to fine space
  virtual gsMatrix<T> prolongation_M(const int& numLevels, vector<memory::shared_ptr<gsMultiBasis<T> > > m_basis, const int& typeLumping, const int& typeBCHandling, gsGeometry<>::Ptr geo, const int& typeProjection) = 0;
   
  /// @brief Prolongate coarse space function to fine space
  virtual gsMatrix<T> restriction_M(const int& numLevels, vector<memory::shared_ptr<gsMultiBasis<T> > > m_basis, const int& typeLumping, const int& typeBCHandling, gsGeometry<>::Ptr geo, const int& typeProjection) = 0;
  ...
  virtual void prolongation(const gsMatrix<T>& Xcoarse, gsMatrix<T>& Xfine, const int& numLevels, const int& numCoarsening, vector<memory::shared_ptr<gsMultiBasis<T> > > m_basis, const int& typeLumping, const int& typeBCHandling, gsBoundaryConditions<T> bcInfo, gsMultiPatch<> mp, gsGeometry<>::Ptr geo, const int& typeProjection, vector<gsSparseMatrix<T>>& m_prolongation_P,vector<gsSparseMatrix<T>>& m_restriction_P, vector<gsMatrix<T>>& m_prolongation_M,vector<gsMatrix<T>>& m_restriction_M,vector<gsSparseMatrix<T>>& m_prolongation_H,vector<gsSparseMatrix<T>>& m_restriction_H, const gsMatrix<>& hp)
  {
    ...
    else
    {
      if(typeLumping == 1)
      {
        ...
        // gsVector<> temp = (M_L_inv).cwiseProduct(Xcoarse);
        // Xfine = m_prolongation_P[numLevels-2]*temp;
